...
#define A_se	p_[0]
#define U_se	p_[1]
#define tau_inac	p_[2]
#define tau_rec	p_[3]
#define effecti_frac_syn	Y_[0]
#define recov_frac_syn	Y_[1]
#define I_app	(A_se*effecti_frac_syn)
#define f_input_aux	(f_input)
#define inact_frac_syn	(1-recov_frac_syn-effecti_frac_syn)
#define f_input	xv_[0]
...
f_[0] = -effecti_frac_syn/tau_inac+U_se*recov_frac_syn*f_input_aux;
f_[1] = inact_frac_syn/tau_rec-U_se*recov_frac_syn*f_input_aux;
...
double initcond(char *varname, double *p_, double *wk_, double *xv_) {
  ...
  if (strcmp(varname, "effecti_frac_syn")==0)
	return gICs[0];
  else if (strcmp(varname, "recov_frac_syn")==0)
	return gICs[1];
  else {
...
int getindex(char *name, double *p_, double *wk_, double *xv_) {
  ...
  if (strcmp(name, "effecti_frac_syn")==0)
	return 0;
  else if (strcmp(name, "recov_frac_syn")==0)
	return 1;
  else if (strcmp(name, "A_se")==0)
	return 2;
  else if (strcmp(name, "U_se")==0)
	return 3;
  else if (strcmp(name, "tau_inac")==0)
	return 4;
  else if (strcmp(name, "tau_rec")==0)
	return 5;
  else {
...
f_[0] = A_se*effecti_frac_syn;
f_[1] = f_input;
f_[2] = 1-recov_frac_syn-effecti_frac_syn;
